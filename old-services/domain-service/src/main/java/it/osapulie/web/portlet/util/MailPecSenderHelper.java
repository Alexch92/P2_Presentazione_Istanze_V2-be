...
//				PrefsPropsUtil.getString("pec.mail.session.mail.smtp.enable"),
	...
	public void sendMail(String subject, 
		...
		log.debug("sendPecMail :: entering method");
		log.debug("sendPecMail :: pecHost : " + pecHost);
		log.debug("sendPecMail :: pecPort : " + pecPort);
		log.debug("sendPecMail :: pecProtocol : " + pecProtocol);
		log.debug("sendPecMail :: pecUsername : " + pecUsername);
		log.debug("sendPecMail :: subject : " + subject);
		log.debug("sendPecMail :: corpo : " + corpo);
		if (attachment != null)
			log.debug(String.format("sendPecMail :: attachments : %s", attachment.length));
...
//			}
			...
			for (int i=0;i<_to.length;i++){
				...
				to[i] = toMail;
				log.debug("sendPecMail :: dest address added : " + em);
			}
			...
			log.debug("sendPecMail :: sending PEC mail...");
	
			MailEngine.send(message);
			
			log.debug("sendPecMail :: PEC succesfully sent!!");
			
		}catch(Exception e){
			log.error("sendPecMail :: " + e.getMessage(),e);
